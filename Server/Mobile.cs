 ...
 *
 *   $Id: Mobile.cs 1058 2013-05-07 19:19:24Z eos@runuo.com $
 *
 ***************************************************************************/
...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
	{
        ...
        #region Scriptiz : permet de définir que le joueur possède un corps d'elfe
        private bool m_IsElfBody = false;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsElfBody
        {
            get { return m_IsElfBody; }
            set
            {
                List<int> bodies = new List<int>() { 400, 401, 402, 403, 605, 606, 607, 608 };
                if (!bodies.Contains(BodyValue))
                {
                    //(AccessLevel.Counselor, e.Mobile.SpeechHue, String.Format("[{0}] {1}", e.Mobile.Name, e.ArgString));
                    foreach (NetState state in NetState.Instances)
                    {
                        Mobile m = state.Mobile;

                        if (m != null && m.AccessLevel >= AccessLevel.GameMaster)
                            m.SendMessage(33, "Mobile incompatible avec l'apparence elfique : " + this.Name + " (" + this.GetType().Name + ")");
                    }
                    return;
                }
                m_IsElfBody = value;
                if (m_IsElfBody)
                    this.BodyValue = (Alive ? Race.Elf.AliveBody(this.Female) : Race.Elf.GhostBody(this.Female));
                else
                    this.BodyValue = (Alive ? this.Race.AliveBody(this.Female) : this.Race.GhostBody(this.Female));
            }
        }
        #endregion

        #region Scriptiz : compteur des morts du mobile
        ...
        #region Scriptiz : système d'hallucinations
		...
		public virtual void AddNameProperties( ObjectPropertyList list )
		{
			...
			if( ShowFameTitle && (m_Player || m_Body.IsHuman) && m_Fame >= 10000 )
				prefix = m_Female ? "La Grande" : "Le Grand";
            ...
            if (guild != null && (m_Player && m_DisplayGuildTitle)) // Scriptiz : on cache le titre et l'abréviation !
			{
				if( suffix.Length > 0 )
        ...
        // Scriptiz : 4 packets de 8 au lieu de 2 de 8 pour le système d'hallucinations !
					...
					if( o is Mobile )
					{
						...
						if( ns != null && Utility.InUpdateRange( m_Location, m.m_Location ) && m.CanSee( this ) )
						{
							...
							if ( ns.StygianAbyss ) {
                                ...
                                // Scriptiz : système d'hallucinations
                                if (m.IsHallucinated)
                                {
                                    p = cache[2][noto];
                                    if (p == null)
                                        cache[2][noto] = p = Packet.Acquire(new MobileMoving(this, noto, m));
                                    // Scriptiz : 3e paramètre pour les hallucinations
                                }
                                else
                                {
                                    p = cache[0][noto];
		...
		public virtual void Resurrect()
		{
			if( !Alive )
			{
                ...
                // Scriptiz : on remet un corps d'elfe s'il faut
                if (this.IsElfBody) Body = Race.Elf.AliveBody(this);
		...
		{
            // Scriptiz : on incrémente de 1 le compteur du nombre de morts du mobile
            this.Deaths++;
			...
			else
			{
                ...
                // Scriptiz : on met un corps d'elfe si le joueur à l'apparence elfique
                if (this.IsElfBody) Body = Race.Elf.GhostBody(this);
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			switch( version )
			{
                // Scriptiz : le compteur du nombre de morts
                case 33:
                    {
                        m_Deaths = reader.ReadInt();
                        goto case 32;
                    }
                // Scriptiz : pour l'apparence elfique
                case 32:
                    {
                        m_IsElfBody = reader.ReadBool();
                        goto case 31;
                    }
				case 31:
					{
		...
		public virtual void Serialize( GenericWriter writer )
		{
            ...
            // Scriptiz [v.33] : on comptabilise le nombre de morts
            writer.Write((int)m_Deaths);

            // Scriptiz [v.32] : pour savoir si le joueur possède un corps d'elfe
            writer.Write((bool)m_IsElfBody);
		...
		{
			return (Body.IsHuman || Body.IsGhost || IsBodyMod ||Body.BodyID == 1253);
		}
		...
		public bool Female
		{
			...
			set
			{
				if( m_Female != value )
				{
                    ...
                    // Scriptiz : si on change female, on modifie le bodyvalue s'il a un corps adapté
                    List<int> bodies = new List<int>() { 400, 401, 402, 403, 605, 606, 607, 608 };
                    if (bodies.Contains(BodyValue))
                    {
                        if (Alive)
                            BodyValue = (IsElfBody ? Race.Elf.AliveBody(m_Female) : this.Race.AliveBody(m_Female));
                        else
                            BodyValue = (IsElfBody? Race.Elf.GhostBody(m_Female) : this.Race.GhostBody(m_Female));
                    }
			...
			if( ourState != null )
			{
                ...
                if (ourState.StygianAbyss)
                {
					if( sendMoving )
					{
						...
						ourState.Send( cache[0][noto] = Packet.Acquire( new MobileMoving( m, noto, this ) ) );
                        // Scriptiz : 3e paramètre pour les hallucinations
					}
					...
					{
						int noto = Notoriety.Compute( m, m );
						ourState.Send( cache[1][noto] = Packet.Acquire( new MobileMovingOld( m, noto, this ) ) );
                        // Scriptiz : 3e paramètre pour les hallucinations
					}
				}
					...
					if( beholder != m && beholder.CanSee( m ) )
					{
						...
						if ( state.StygianAbyss ) {
							if( sendMoving )
							{
								...
								if( p == null )
									cache[0][noto] = p = Packet.Acquire( new MobileMoving( m, noto, this ) );
                                // Scriptiz : 3e paramètre pour les hallucinations
							...
							if( sendMoving || sendHealthbarPoison || sendHealthbarYellow )
							{
								...
								if( p == null )
									cache[1][noto] = p = Packet.Acquire( new MobileMovingOld( m, noto, this ) );
                                // Scriptiz : 3e paramètre pour les hallucinations
