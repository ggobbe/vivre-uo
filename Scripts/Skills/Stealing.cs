	...
	public class Stealing
	{
		...
		private class StealingTarget : Target
		{
			...
			private Item TryStealItem( Item toSteal, ref bool caught )
			{
                ...
                // Scriptiz : les objets marqués non stealables ne peuvent être volés !
                if (toSteal != null && !toSteal.Stealable)
                {
                    m_Thief.SendMessage("Vous ne pouvez pas voler cet objet.");
                }
			...
			{
                if ((int)from.Skills[SkillName.Stealing].Value < 99)
				...
				if ( caught )
				{
                    // Si le voleur est prit, on le révèle !
					...
					else if ( root is Mobile )
					{
						...
						foreach ( NetState ns in m_Thief.GetClientsInRange( 8 ) )
						{
                            ...
                            if (ns.Mobile != m_Thief && ns.Mobile.Skills[SkillName.DetectHidden].Value > m_Thief.Skills[SkillName.Hiding].Value - 20)
                                m_Thief.RevealingAction();
						}
					}
				}
