...
namespace Server.Items
{
    ...
    public abstract class BaseClothing : BaseWearable, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability
	{
		#region Factions
        ...
        public int ClothingDifficulty
        {
            get
            {
                ...
                if (Attributes.WeaponSpeed != 0)
                    difficulty++;
                if (ClothingAttributes.DurabilityBonus != 0)
                    difficulty++;
                if (ClothingAttributes.LowerStatReq != 0)
                    difficulty++;
                if (ClothingAttributes.MageArmor != 0)
                    difficulty++;
                if (ClothingAttributes.SelfRepair != 0)
                    difficulty++;
		...
		public override bool CanEquip( Mobile from )
		{
			...
			if( from.AccessLevel < AccessLevel.GameMaster )
			{
                ...
                {
                    from.SendMessage("Ce vêtement vous semble inconnu");
                    return false;
                }
				else
				{
		...
		public override void Deserialize( GenericReader reader )
		{
            ...
            if (version < 6)
                m_Identified = true; // we don't know, so, assume it's identfied
