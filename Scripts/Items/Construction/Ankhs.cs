		...
		private class TitheEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
                if (m_Mobile.CheckAlive())
                {
                    ...
                    if (m_Mobile is PlayerMobile && ((PlayerMobile)m_Mobile).IsArtedarKnight)  // Vinds :Les Chevaliers d'artedar ne gagnent pas de tithe par le don d'or

                        m_Mobile.SendMessage("Artedar n'a que faire de cette absurde Piété !!");


                    else m_Mobile.SendGump(new TithingGump(m_Mobile, 0));
                }
			}
		}
	}
