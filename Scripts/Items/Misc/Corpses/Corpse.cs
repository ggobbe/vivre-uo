...
namespace Server.Items
{
	...
	public class Corpse : Container, ICarvable
	{
        ...
        // Scriptiz : on double le temps de decay (de 7 à 15min par phase de decay)
		private static TimeSpan m_DefaultDecayTime = TimeSpan.FromMinutes( 30.0 );
		private static TimeSpan m_BoneDecayTime = TimeSpan.FromMinutes( 30.0 );
        ...
        // Scriptiz : on log les loots !
        public void LogLoot(string msg)
        {
            ...
            try
            {
                StreamWriter writer = new StreamWriter(Path.Combine(directory, String.Format("Loots-{0}.log", DateTime.Now.ToLongDateString())), true);
            ...
            {
                Console.WriteLine("Corpse.LogLoot : " + ex.Message);
            }
        }
		...
		{
            // Scriptiz : Les objets marqués non Lootable ne peuvent pas être lootés
            if (item != null && !item.Lootable && Owner != null && Owner.Player)
            {
                from.SendMessage("Vous ne pouvez pas looter cet objet.");
                return false;
            }
