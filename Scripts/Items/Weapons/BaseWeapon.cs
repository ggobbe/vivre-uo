...
namespace Server.Items
{
    ...
    public abstract class BaseWeapon : BaseWearable, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
	{
        ...
        public int WeaponDifficulty
        {
            ...
            if (Attributes.WeaponSpeed != 0)
                difficulty++;
            if (WeaponAttributes.DurabilityBonus != 0)
                difficulty++;
            if (WeaponAttributes.HitColdArea != 0)
                difficulty++;
            if (WeaponAttributes.HitDispel != 0)
                difficulty++;
            if (WeaponAttributes.HitEnergyArea != 0)
                difficulty++;
            if (WeaponAttributes.HitFireArea != 0)
                difficulty++;
            if (WeaponAttributes.HitFireball != 0)
                difficulty++;
            if (WeaponAttributes.HitHarm != 0)
                difficulty++;
            if (WeaponAttributes.HitLeechHits != 0)
                difficulty++;
            if (WeaponAttributes.HitLeechMana != 0)
                difficulty++;
            if (WeaponAttributes.HitLeechStam != 0)
                difficulty++;
            if (WeaponAttributes.HitLightning != 0)
                difficulty++;
            if (WeaponAttributes.HitLowerAttack != 0)
                difficulty++;
            if (WeaponAttributes.HitLowerDefend != 0)
                difficulty++;
            if (WeaponAttributes.HitMagicArrow != 0)
                difficulty++;
            if (WeaponAttributes.HitPhysicalArea != 0)
                difficulty++;
            if (WeaponAttributes.HitPoisonArea != 0)
                difficulty++;
            if (WeaponAttributes.LowerStatReq != 0)
                difficulty++;
            if (WeaponAttributes.MageWeapon != 0)
                difficulty++;
            if (WeaponAttributes.ResistColdBonus != 0)
                difficulty++;
            if (WeaponAttributes.ResistEnergyBonus != 0)
                difficulty++;
            if (WeaponAttributes.ResistFireBonus != 0)
                difficulty++;
            if (WeaponAttributes.ResistPhysicalBonus != 0)
                difficulty++;
            if (WeaponAttributes.ResistPoisonBonus != 0)
                difficulty++;
            if (WeaponAttributes.SelfRepair != 0)
                difficulty++;
		...
		public int GetDurabilityBonus()
		{
			...
			{
				case WeaponDurabilityLevel.Durable: bonus += 10; break;
				case WeaponDurabilityLevel.Substantial: bonus +=20; break;
				case WeaponDurabilityLevel.Massive: bonus += 30; break;
				case WeaponDurabilityLevel.Fortified: bonus += 45; break;
				case WeaponDurabilityLevel.Indestructible: bonus += 60; break;
			}
		...
		public override bool CanEquip( Mobile from )
		{
            ...
            {
                from.SendMessage("Cette arme vous semble inconnue");
                return false;
            }
		...
		public virtual bool CheckHit( Mobile attacker, Mobile defender )
		{
            ...
            //Ajout de attack chance silver vs necro
            if (Resource == CraftResource.MSilver && context != null && context.Spell is NecromancerSpell)
                bonus += 5; 

            //Plume : Ajout de hit hance pour les filles sur les hommes si Arme en Rose
            if (Resource == CraftResource.MRose && attacker.Female && (!defender.Female))
            {
                bonus += 10;
            }
		...
		public static bool CheckParry( Mobile defender )
		{
            ...
            if ( shield != null )
			{
                ...
                // Ajout defense silver vs necro

                if (shield.Resource == CraftResource.MSilver)
                    chance += 0.05;
		...
		public virtual int AbsorbDamageAOS( Mobile attacker, Mobile defender, int damage )
		{
			...
			if ( defender.Player || defender.Body.IsHuman )
			{
                ...
                //Plume : 2e chance pour les filles si le bouclier est en rose
                BaseShield shield = defender.FindItemOnLayer(Layer.TwoHanded) as BaseShield;
                
                if (!blocked && shield != null && shield.Resource == CraftResource.MRose && defender.Female && !(attacker.Female))
                 {
                     blocked = Utility.RandomDouble() > 0.10 ? false : true;
                 }
		...
		public virtual void OnHit( Mobile attacker, Mobile defender, double damageBonus )
		{
			...
			TransformContext context = TransformationSpellHelper.GetContext( defender );
            // Ajout dégats contre nécro avec silver
            if (Resource == CraftResource.MSilver && context != null && context.Spell is NecromancerSpell)
                percentageBonus += 15; 
		...
		public virtual int GetDamageBonus()
		{
			...
			switch ( m_Quality )
			{
				...
				case WeaponQuality.Exceptional:	bonus += 15; break;
			}
			...
			switch ( m_DamageLevel )
			{
				...
				case WeaponDamageLevel.Force:	bonus += 15; break;
				case WeaponDamageLevel.Power:	bonus += 20; break;
				case WeaponDamageLevel.Vanq:	bonus += 25; break;
			}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			SetSaveFlag( ref flags, SaveFlag.Resource,			m_Resource != CraftResource.MIron );
		...
		{
            //Plume: On affiche que le nom de l'item
            ...
            //    case CraftResource.OakWood: oreType = "Oak"; break; // red
            //    case CraftResource.YewWood: oreType = "Yew"; break; // yellow
            //    case CraftResource.Bloodwood: oreType = "Bloodwood"; break; // black
            //    case CraftResource.Frostwood: oreType = "Frostwood"; break; // green
            //    case CraftResource.Heartwood: oreType = "Heartwood"; break; // white
		...
		public int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			...
			if ( Core.AOS )
			{
                ...
                //Ajout d'un scalar pour chaque métal pour les différents types de durabilité/dégats/accuités
                double DurScalar = 1;
                double DmgScalar = 1;
                double AccScalar = 1;
                bool Daylight = false;
                ...
                switch (Resource)
					{
						...
						case CraftResource.MGold:
						{
                            DmgScalar = 0.95;
                            AccScalar = 0.95;
                            break;
						}
						case CraftResource.MCopper:
						{
                            DurScalar = 0.98;
                            DmgScalar = 0.98;
                            AccScalar = 0.99;
                        ...
                        case CraftResource.MOldcopper:
						{
                            DmgScalar = 0.99;
                            AccScalar = 1.01;
                            break;
						}
						case CraftResource.MDullcopper:
						{
                            DurScalar = 1.01;
                            break;
						}
						case CraftResource.MSilver:
						{
                            DurScalar = 1.05;
                            AccScalar = 1.02;
                            break;
						}
						case CraftResource.MShadow:
						{
                            DmgScalar = 0.95;
                            AccScalar = 1.05;
							break;
						}
						case CraftResource.MBloodrock:
						{
                            break;
						}
                        case CraftResource.MBlackrock:
                        {
                            break;
                        }
                        case CraftResource.MMytheril:
                        {
                            break;
                        }
                        case CraftResource.MRose:
                        {
                            AccScalar = 1.03;
                            break;
                        }
                        case CraftResource.MVerite:
                        {
                            DurScalar = 1.05;
                            DmgScalar = 1.05;
                            AccScalar = 1.05;
                        ...
                        case CraftResource.MAgapite:
                        {
                            
                            break;
                        }
                        case CraftResource.MRusty:
                        {
                            DurScalar = 0.90;
                            DmgScalar = 0.95;
                            AccScalar = 0.80;
                        ...
                        case CraftResource.MCrystaline:
                        {
                            
                            break;
                        }
                        case CraftResource.MKrynite:
                        {
                           
                            break;
                        }
                        case CraftResource.MVulcan:
                        {
                            
                            break;
                        }
                        case CraftResource.MBloodcrest:
                        {
                           
                            break;
                        }
                        case CraftResource.MElvin:
                        {
                      
                            break;
                        }
                        case CraftResource.MAcid:
                        {
                            
                            break;
                        }
                        case CraftResource.MAqua:
                        {
                         
                            break;
                        }
                        case CraftResource.MEldar:
                        {
                            DurScalar = 1.02;
                            DmgScalar = 1.03;
                            AccScalar = 1.15;
                        ...
                        case CraftResource.MGlowing:
                        {
                            DurScalar = Daylight? 1.04:0.80;
                            DmgScalar = Daylight? 1.04 : 0.80;
                            AccScalar = Daylight? 1.04 : 0.80;
                            break;
                        }
                        case CraftResource.MGorgan:
                        {

                            break;
                        }
                        case CraftResource.MSandrock:
                        {
 
                            break;
                        }
                        case CraftResource.MSteel:
                        {
                ...
                double BonusStr = Math.Sqrt(from.Str);
                double BonusSkills = Math.Sqrt(from.Skills[SkillName.ArmsLore].Value + from.Skills[SkillName.Blacksmith].Value);
                int BonusVulcan = Math.Max(0,(int)from.FireResistance-from.ColdResistance/30);
                double DurChance = (BonusStr + BonusSkills) * Math.Pow(DurScalar,2);
                double DmgChance = (BonusStr + BonusSkills) * Math.Pow(DmgScalar, 2);
                double AccChance = (Math.Sqrt(from.Dex) + BonusSkills) * Math.Pow(AccScalar, 2);
                int DurRetry = (int)(Math.Min(1.08,DurScalar) * 100 - 100) + 2 + BonusVulcan;
                int AccRetry = (int)(Math.Min(1.08, AccScalar) * 100 - 100) + 2 + BonusVulcan;
                int DmgRetry = (int)(Math.Min(1.08, DmgScalar) * 100 - 100) + 2 + BonusVulcan;
                int a = 0;
                int b = 0;
                int c = 0;
                for ( int i = 0; i < DurRetry;)
                    {
                        if ((DurChance-a*3) > Utility.Random(100+(a*15)))
                             {
                            //Console.WriteLine("Dur : {0} sur {1} : {2} ", i, DurRetry, DurabilityLevel);
                            DurabilityLevel += 1;
                            a++;
                            }
                        else
                ...
                for (int f = 0; f < DmgRetry; )
                {
                    if ((DmgChance-b*3) > Utility.Random(100+(b*15)))
                        {
                            //Console.WriteLine("Dmg: {0} sur {1} : {2} ", f, DmgRetry, DamageLevel);
                            DamageLevel += 1;
                            b++;
                            }
                    else
                ...
                for (int e = 0; e < AccRetry; )
                {
                    //Console.WriteLine("Accu: {0} sur {1} : {2} ", e, DmgRetry, AccuracyLevel);
                    if ((AccChance-c*3) > Utility.Random(100+ (c*15)))
                        {
                            AccuracyLevel += 1;
                            c++;
                            }
