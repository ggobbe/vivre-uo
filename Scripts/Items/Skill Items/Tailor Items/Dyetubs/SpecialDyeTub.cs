...
namespace Server.Items
{
    public class SpecialDyeTub : DyeTub, IRewardItem
    {
		...
		{
            // Scriptiz : blessed uniquement si item reward
		...
		public override void OnDoubleClick( Mobile from )
		{
            ...
            if (Charges <= 0)
            {
                from.SendMessage("Ce bac est vide");
                return;
            }
            if (!from.CheckSkill(SkillName.Tailoring, -5, 50))
            {
                from.SendMessage("Vous ratez votre essais");
                Charges--;
            }
