...
namespace Server.Gumps
{
	public class RunebookGump : Gump
	{
		...
		private void AddIndex()
		{
			...
			AddHtml( 158, 22, 100, 18, "Renommer le livre", false, false ); // Rename book
		...
		private class InternalPrompt : Prompt
		{
			...
			{
				from.SendMessage( "Requête annulée" ); // Request cancelled.
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( buttonID == 1 ) // Rename book
			{
				...
				{
					from.SendMessage( "Choisissez le titre de votre livre de runes" ); // Please enter a title for the runebook:
				...
				else
				{
                    ...
                    from.SendMessage("Vous ne pouvez faire cela lorsque le livre est fixé au sol", null, 0x35); // That cannot be done while the book is locked down.
				}
			}
			else
			{
					...
					switch ( type )
					{
						case 0: // Use charges
						{
							if ( m_Book.CurCharges <= 0 )
							{
								...
								from.SendMessage( "Il n'y a plus de charge dans ce livre" ); // There are no charges left on that item.
							}
							else
							{
								int xLong = 0, yLat = 0;
						...
						case 3: // Recall
						{
							...
							{
								from.SendMessage( "Vous n'avez pas ce sort!" ); // You do not have that spell!
							}
