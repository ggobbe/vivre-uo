...
{
    public class CarrotSeed : BaseCrop
    {
    ...
    public class CarrotSeedling : BaseCrop
    {
        ...
        [Constructable]
        public CarrotSeedling(Mobile sower)
            : base(0xC68)
        {
            Movable = false;
            Name = AgriTxt.Seedling + " de Carottes";
            m_sower = sower;

            init(this);
        }

        public static void init(CarrotSeedling plant)
        {
            plant.thisTimer = new CropHelper.GrowTimer(plant, typeof(CarrotCrop), plant.Sower);
        ...
        public CarrotSeedling(Serial serial)
            : base(serial)
        {
        }
    ...
    public class CarrotCrop : BaseCrop
    {
        ...
        public static void init(CarrotCrop plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill and sower
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage(AgriTxt.YouPick + " {0} carotte{1}!", pick, (pick == 1 ? "" : "s"));
        ...
        {
            private CarrotCrop i_plant;

            public CropTimer(CarrotCrop plant)
