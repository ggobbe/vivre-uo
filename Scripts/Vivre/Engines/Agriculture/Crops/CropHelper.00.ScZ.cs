...
{ 
	public class BaseCrop : Item
	{
		...
		public virtual bool CanGrowDirt{ get{ return false; } }  // false
        public virtual bool CanGrowGround { get { return false; } }    // false
        public virtual bool CanGrowSwamp { get { return false; } } // false
        public virtual bool CanGrowSand { get { return false; } }  // false
	...
	{ 
		...
		{
		// Je mets un try... catch pour etre sur que ca plante pus
			try
			{
				...
				// L'erreur est surment ici
					ground = ( tileID >= FarmTiles[i] && tileID <= FarmTiles[i + 1] );

				return ground;
			} catch 
			{
			// Il y a une erreur dans cette fonction 
			// Un Out of Range
				return false;
			}
		}
		...
		{ 
			...
			{ 
				...
				lumberValue = sower.Skills[SkillName.Lumberjacking].Value / 100;
			} 
			...
			{ 
				if ( cnt++ / 100 > rnd ) // between 10 and 30 minutes
				{
					...
					{ 
						...
						if ( newitem == null || Utility.RandomDouble() > lumberValue )
						{
