...
{
    public class HoublonSeed : BaseCrop
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new HoublonSeedling(from);
            item.Location = m_pnt;
            item.Map = m_map;

        }

        public HoublonSeed(Serial serial)
            : base(serial)
        {
        }
    ...
    public class HoublonSeedling : BaseCrop
    {
        ...
        [Constructable]
        public HoublonSeedling(Mobile sower)
            : base(Utility.RandomList(0xC60, 0xC5E))
        {
            Movable = false;
            Name = AgriTxt.Seedling + " de Houblon";
            m_sower = sower;

            init(this);
        }

        public static void init(HoublonSeedling plant)
        {
            plant.thisTimer = new CropHelper.GrowTimer(plant, typeof(HoublonCrop), plant.Sower);
        ...
        public HoublonSeedling(Serial serial)
            : base(serial)
        {
        }
    ...
    public class HoublonCrop : BaseCrop
    {
        ...
        [Constructable]
        public HoublonCrop(Mobile sower)
            : base(Utility.RandomList(0xC60, 0xC5E))
        {
            Movable = false;
            Name = "Plant de Houblon";
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill and sower
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage(AgriTxt.YouPick + " {0} houblon{1}!", pick, (pick == 1 ? "" : "s"));
        ...
        public HoublonCrop(Serial serial)
            : base(serial)
        {
        }
