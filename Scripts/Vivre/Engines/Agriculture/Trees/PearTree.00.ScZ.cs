...
namespace Server.Items.Crops
{
	...
	//naga
		public class PearSeed : BaseCrop 
		{ 
		...
		[Constructable]
		public PearSeed() : this( 1 )
		{
		}

		[Constructable]
		public PearSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		{ 
			...
			this.Consume(); 
			Item item = new PearSapling();// from ); 
			item.Location = m_pnt; 
			item.Map = m_map; 
			
		} 

		public PearSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		{ 
			Movable = false; 
			Name = AgriTxt.Seedling + " de Poirier"; 
	...
	public class PearTree : BaseTree
	{
		...
		[Constructable] 
		public PearTree( Point3D pnt, Map map ) : base( Utility.RandomList( 0xDAA, 0xDA6 ) ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage(AgriTxt.YouPick + " {0} poire{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
