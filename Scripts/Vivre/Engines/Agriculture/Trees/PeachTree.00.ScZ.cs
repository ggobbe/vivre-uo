...
namespace Server.Items.Crops
{
	...
	//naga
		public class PeachSeed : BaseCrop 
		{ 
		...
		[Constructable]
		public PeachSeed() : this( 1 )
		{
		}

		[Constructable]
		public PeachSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		{ 
			...
			this.Consume(); 
			Item item = new PeachSapling();// from ); 
			item.Location = m_pnt; 
			item.Map = m_map; 
			
		} 

		public PeachSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		{ 
			Movable = false; 
			Name = AgriTxt.Seedling + " de Pêcher"; 
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage(AgriTxt.YouPick + " {0} pêche{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
