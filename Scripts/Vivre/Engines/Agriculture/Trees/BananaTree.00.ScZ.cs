...
namespace Server.Items.Crops
{
    ...
    public class BananaSeed : BaseCrop
    {
        ...
        public BananaSeed(Serial serial)
            : base(serial)
        {
        }
    ...
    public class BananaSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = AgriTxt.Seedling + " de Bananier";

            init(this);
        }

        public static void init(BananaSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(BananaTree), delay);
    ...
    public class BananaTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public BananaTree(Point3D pnt, Map map)
            : base(Utility.RandomList(0xCAA, 0xCA8)) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
            Name = "Bananier";
            int trunkID = 0xCAB; //((Item)this).ItemID - 2;
        ...
        public static void init(BananaTree plant, bool full)
        {
        ...
        {
            private BananaTree i_plant;

            public FruitTimer(BananaTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 2))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Banana fruit = new Banana((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(BananaTree));
        ...
        {
            private BananaTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(BananaTree Tree, FruitTreeStump Stump, Mobile from)
