...
namespace Server
{
	...
	public class FurnitureAttribute : Attribute
	{
		...
		{
            // Scriptiz : pour autoriser le dye sur les fournitures Addon !
            if (item is AddonComponent && item != null)
            {
                AddonComponent acc = item as AddonComponent;

                return (acc.Addon != null && acc.Addon.GetType().IsDefined(typeof(FurnitureAttribute), false));
            }
            // Scriptiz : fin modif
