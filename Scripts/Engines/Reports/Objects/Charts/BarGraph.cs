...
namespace Server.Engines.Reports
{
	...
	public class BarGraph : Chart
	{
		...
		{
			BarGraph barGraph = new BarGraph( valueName + " over time", "graphs_" + valueName.ToLower() + "_ot", 10, "Heure (QC)", valueName, BarGraphRenderMode.Lines );
			...
			for ( int i = 0; i < history.Snapshots.Count; ++i )
			{
				...
				//barGraph.Items.Add( hours + (realHours >= 12 ? " PM" : " AM"), val );
                barGraph.Items.Add(realHours + "h", val); // Scriptiz : affichage 24h
			}
