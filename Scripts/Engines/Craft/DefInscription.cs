    ...
    public class DefInscription : CraftSystem
    {
        ...
        private void AddSpell(Type type, params Reg[] regs)
        {
            ...
            switch (m_Circle)
            {
                ...
                case 6: minSkill = 60.7; maxSkill = 120.7; break;   // Scirptiz : défaut = 110.7
                case 7: minSkill = 75.0; maxSkill = 160.0; break;   // Scriptiz : défaut = 125.0
            }

            int amount = (m_Circle >= 7 ? 2 : 1);  // Scriptiz : par défaut 1 de chaque réactif, maintenant 2 réactifs pour cercle 8 et 1 réactif pour les autres
            int index = AddCraft(type, 1044369 + m_Circle, 1044381 + m_Index++, minSkill, maxSkill, m_RegTypes[(int)regs[0]], 1044353 + (int)regs[0], amount, 1044361 + (int)regs[0]);

            for (int i = 1; i < regs.Length; ++i)
                AddRes(index, m_RegTypes[(int)regs[i]], 1044353 + (int)regs[i], amount, 1044361 + (int)regs[i]);

            AddRes(index, typeof(BlankScroll), 1044377, 1, 1044378);

            int mana = (int)(m_Mana * 1.2); // Scriptiz : 20% de mana nécessaire en plus
        ...
        public override void InitCraftList()
        {
            ...
            // Scriptiz : les parchos nécros ne peuvent pas être craftés !
            /*
			if ( Core.SE )
			{
			...
			*/
            ...
            index = AddCraft(typeof(BottleInscriber), 1044294, "Étiquette pour bouteille", 45.0, 95.0, typeof(BlankScroll), 1044377, 3, 1044378);
            AddRes(index, typeof(Beeswax), "Cire d'abeille", 1, "Il vous manque de la Cire d'abeille");

            index = AddCraft(typeof(IDWand), 1044294, "Baguette d'identification", 40.0, 80.0, typeof(DispelScroll), "Dispel Scroll", 1, "Vous n'avez pas de parchemin de dissipation");
            AddRes(index, typeof(BlankWand), "Baguette de bois", 1, "Vous n'avez pas de baguette en bois");

            index = AddCraft(typeof(VoodooDollFemale), 1044294, "Poupée maléfique (Femme)", 75.0, 105.0, typeof(PoupeeFemale), "Poupée de femme", 1, "Vous n'avez pas de poupée femme");
            AddRes(index, typeof(MassCurseScroll), "Sort de malédiction de masse", 1, "Vous n'avez pas ce sort");
            AddSkill(index, SkillName.SpiritSpeak, 25.0, 50.0);
            
            index = AddCraft(typeof(VoodooDollMale), 1044294, "Poupée maléfique (Homme)", 75.0, 105.0, typeof(PoupeeMale), "Poupée d'homme", 1, "Vous n'avez pas de poupée homme");
            AddRes(index, typeof(MassCurseScroll), "Sort de malédiction de masse", 1, "Vous n'avez pas ce sort");
            AddSkill(index, SkillName.SpiritSpeak, 25.0, 50.0);
            
            if (Core.AOS)
            {
                index = AddCraft(typeof(Engines.BulkOrders.BulkOrderBook), "Reliures", 1028793, 65.0, 115.0, typeof(BlankScroll), 1044377, 10, 1044378);
            ...
            if (Core.SE)
            {
                index = AddCraft(typeof(Spellbook), "Reliures", 1023834, 50.0, 126, typeof(BlankScroll), 1044377, 10, 1044378);
                AddRes(index, typeof(Leather), 1044462, 1, 1044463);
            }

            index = AddCraft(typeof(BlueBook), "Reliures", "Livre bleu", 20, 80, typeof(BlankScroll), 1044377, 5, 1044378);
            AddRes(index, typeof(Leather), 1044462, 1, 1044463);

            index = AddCraft(typeof(TanBook), "Reliures", "Livre beie", 20, 80, typeof(BlankScroll), 1044377, 5, 1044378);
            AddRes(index, typeof(Leather), 1044462, 1, 1044463);

            index = AddCraft(typeof(RedBook), "Reliures", "Livre bleu", 20, 80, typeof(BlankScroll), 1044377, 5, 1044378);
            AddRes(index, typeof(Leather), 1044462, 1, 1044463);

            index = AddCraft(typeof(BrownBook), "Reliures", "Livre brun", 20, 80, typeof(BlankScroll), 1044377, 5, 1044378);
            AddRes(index, typeof(Leather), 1044462, 1, 1044463);
