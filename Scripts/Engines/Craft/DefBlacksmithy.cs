	...
	public class DefBlacksmithy : CraftSystem
	{
		...
		public override void InitCraftList()
		{
			...
			*/
			...
			AddCraft( typeof( PlateChest ), 1011078, 1046431, 75.0, 122.0, typeof( IronIngot ), 1044036, 25, 1044037 );
			...
			if( Core.SE )
			{
                ...
                // Scriptiz : les diadèmes sont des bijoux craftables par les thinkers (Diademe & DiademeDecore)
                // Scriptiz : on remet les circlet mais on augmente le metal nécessaire et on descend les skills afin d'éviter le campage
				/*if( Core.ML )
				{
					//index = AddCraft( typeof( Circlet ), 1011079, 1032645, 62.1, 112.1, typeof( IronIngot ), 1044036, 6, 1044037 );
                    index = AddCraft(typeof(Circlet), 1011079, 1032645, 62.6, 112.6, typeof(IronIngot), 1044036, 15, 1044037);
					SetNeededExpansion( index, Expansion.ML );

					//index = AddCraft( typeof( RoyalCirclet ), 1011079, 1032646, 70.0, 120.0, typeof( IronIngot ), 1044036, 6, 1044037 );
                    index = AddCraft(typeof(RoyalCirclet), 1011079, 1032646, 70.0, 120.0, typeof(IronIngot), 1044036, 20, 1044037);
					SetNeededExpansion( index, Expansion.ML );

					//index = AddCraft( typeof( GemmedCirclet ), 1011079, 1032647, 75.0, 125.0, typeof( IronIngot ), 1044036, 6, 1044037 );
                    index = AddCraft(typeof(GemmedCirclet), 1011079, 1032647, 75.0, 125.0, typeof(IronIngot), 1044036, 20, 1044037);
            ...
            //Myron : Ajout Paladin Sword 
            if (Core.AOS)
                AddCraft(typeof(PaladinSword), 1011081, 1029934, 40.1, 90.1, typeof(IronIngot), 1044036, 20, 1044037);
			if ( Core.AOS )
			...
			if( Core.SE )
			{
				...
				if( Core.ML )
				{
                    ...
                    // Scriptiz : on laisse commenté car pas les recettes
            ...
            AddCraft(typeof(ThrowingDagger), "Throwing", "Throwing Dagger", 10, 30, typeof(IronIngot), 1044036, 2, 1044037);
            AddCraft(typeof(ThrowingAxe), "Throwing", "Throwing Axe", 25, 35, typeof(IronIngot), 1044036, 6, 1044037);
            ...
            AddSubRes(typeof(AgapiteIngot), "Agapite", 55.0, 1044268);
