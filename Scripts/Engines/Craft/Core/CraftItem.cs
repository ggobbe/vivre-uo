...
namespace Server.Engines.Craft
{
	...
	public class CraftItem
	{
        ...
        //Plume: Ajout Force non low quality
        private bool m_ForceNonLow;

        public bool ForceNonLow
        {
            get { return m_ForceNonLow; }
            set { m_ForceNonLow = value; }
        }

        private bool m_TopChance;

        public bool TopChance
        {
            get { return m_TopChance; }
            set { m_TopChance = value; }
        }

        private double m_MaxChance;

        public double MaxChance
        {
            get { return m_MaxChance; }
            set { m_MaxChance = value; }
        }
		...
		private static Type[] m_ColoredItemTable = new Type[]
			{
                ...
                // Scriptiz : quelques objets qui ne sont plus dyable avec le furniture dye tub
                // mais qui retienne la couleur du bois utilisé
                typeof(WoodenBench),
                typeof(WoodenChair),
                typeof(BambooChair),
                typeof(Stool),
                ...
                typeof(Armoire),
                typeof(FancyArmoire),
                typeof(CommodityDeedBox),

                // Parchemins et chaises elfiques
                typeof(BlankScroll),
        ...
        private static Type[] m_ColoredResourceTable = new Type[]
			{
                ...
                typeof(BaseWoodBoard), typeof(BaseLog),
		...
		private static Type[] m_MarkableTable = new Type[]
				{
					...
					typeof( BaseQuiver ), typeof(BaseJewel)
				};
        ...
        public double GetLowChance(CraftSystem system, double chance, Mobile from)
        {
            if (m_ForceNonLow)
                return 1.1;
            ...
            {
                default:
                //Plume : À partir de 50.1% de chance de réussir l'item, il ne sera jamais LowQuality
                case CraftECA.ChanceMinusSixty: chance += 0.5; break;
                //Plume : À partir de 60% de chance de réussir l'item, il se sera jamais LowQuality
                case CraftECA.FiftyPercentChanceMinusTenPercent: chance *= 1.5 +0.1; break;
                //Plume : Fonctionnelle selon l'offset
                case CraftECA.ChanceMinusSixtyToFourtyFive:
                    {
                        double offset = 0.60 - ((from.Skills[system.MainSkill].Value - 95.0) * 0.03);
		...
		public bool CheckSkills( Mobile from, Type typeRes, CraftSystem craftSystem, ref int quality, ref bool allRequiredSkills, bool gainSkills )
		{
            ...
            if (GetLowChance(craftSystem, chance, from) < Utility.RandomDouble())
                quality = 0;
