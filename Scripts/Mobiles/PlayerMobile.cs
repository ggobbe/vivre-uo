...
namespace Server.Mobiles
{
	...
	public enum PlayerFlag // First 16 bits are reserved for default-distro use, start custom flags at 0x00010000
	{
        ...
        // Scriptiz : ajouts de flags pour le système de duels en arène
        isinchal = 0x00100001,
	...
	{
        #region Scriptiz : mise à jour du gump d'alimentation
        [CommandProperty(AccessLevel.GameMaster)]
        public override int Hunger
        {
            get { return base.Hunger; }
            set
            {
                base.Hunger = value;
                Alimentation.UpdateGump(this);
            }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public override int Thirst
        {
            get { return base.Thirst; }
            set
            {
                base.Thirst = value;
                Alimentation.UpdateGump(this);
        ...
        #region Scriptiz : ajout de l'alignement sur Malas
        private Alignment m_Alignment = Alignment.Neutral;
        ...
        #region Vinds : ajout des chevaliers d'artedar
        private bool m_IsArtedarKnight = false;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsArtedarKnight
        {
            get { return m_IsArtedarKnight; }
            set { m_IsArtedarKnight = value; }
        }
        #endregion

        #region Scriptiz : Ajout pour le système d'hallucinations
        private bool m_Hallucinating;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool Hallucinating
        {
            get { return m_Hallucinating; }
            set
            {
                if (m_Hallucinating == value)
                    return;


                if (value) //PRIMA INIZIALIZZIAMO
                {
                    m_Hallucinating = value; // e poi settiamo la var..
                    this.SendEverything();
                }
                else //DEINIZIALIZZAZIONE
                {
                    m_Hallucinating = value; // settiamo subito la var
                    this.SendEverything();
                }
        ...
        {
            public ReseedTimer()
                : base(TimeSpan.FromSeconds(40), TimeSpan.FromSeconds(20))
            {
		...
		*/
        ...
        #region Scriptiz : pour le système de duels
        ...
        // Scriptiz : bool pour afficher ou cacher le GM 
        public bool ShowInStatus = true;
        
        // Scriptiz : bool pour les persos fées
        private bool m_IsFairy = false;
        
        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsFairy
        {
        	get { return m_IsFairy; }
        	set { m_IsFairy = value; }
        }

        // Scriptiz : bool pour nager sur un cheval
        [CommandProperty(AccessLevel.GameMaster)]
        public override bool CanSwim
        {
            // base sinon infini
            get { return base.CanSwim || (Mount is SeaHorse); }
            set { base.CanSwim = value; }
        }

        // Scriptiz : les autres joueurs ne peuvent pas ouvrir le paperdoll d'une fille transformée
        public override bool CanPaperdollBeOpenedBy(Mobile from)
        {
            if (this.Female && this.BodyMod != 0 && from != this && from.AccessLevel == AccessLevel.Player)
            {
                from.SendMessage("Vous ne pouvez pas ouvrir ce paperdoll.");
                return false;
            }
        ...
        {
            get { return BaseCandyCane.GetToothAche(this); }
            set { BaseCandyCane.SetToothAche(this, value); }
        }
		...
		private static void OnLogin( LoginEventArgs e )
		{
            ...
            // Scriptiz : On evoie le gump d'alimentation lors de la connection
            Alimentation.SendGump(from);
		}
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
            ...
            // Scriptiz : Ajout des menus contextuels pour mettre en prison un joueur
		...
		public override void OnDeath( Container c )
		{
            ...
            // Plume : on log les morts            
            string sKilled = String.Format("{0} ({1})", this.Name, (this.Account != null ? this.Account.Username : "Aucun"));

            string sKiller = "un pouvoir inconnu...";
            if(killer != null)
                sKiller = String.Format("{0} ({1})", killer.Name, (killer.Account != null ? killer.Account.Username : killer.GetType().FullName));

            LogDeath(String.Format(DateTime.Now + ": {0} a été tué par {1}", sKilled, sKiller));
		}

        // Plume : Méthode pour logger les morts
        public static void LogDeath(string msg)
        {
            if (!Directory.Exists("Logs"))
                Directory.CreateDirectory("Logs");

            string directory = "Logs/Death";
            ...
            try
            {
                StreamWriter writer = new StreamWriter(Path.Combine(directory, String.Format("Death-{0}.log", DateTime.Now.ToLongDateString())), true);
            ...
            {
                Console.WriteLine("PlayerMobile.LogDeath : " + ex.Message);
            }
        }
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            if (Addiction == null)
                Addiction = new double[Enum.GetValues(typeof(PotionEffect)).Length];
            writer.Write((int)Addiction.Length);

           for (int i = 0; i < Addiction.Length; i++)
                {
                if(Addiction[i] == 0)
                    {
                    writer.Write(Addiction[i]);
                    continue;
                    }
            ...
            //Vinds : chevaliers d'artedar
            writer.Write((bool)m_IsArtedarKnight);
			
			// Scriptiz : alignement sur Malas
			writer.Write((int)m_Alignment);
			
			// Scriptiz : pour les fées
			writer.Write((bool)m_IsFairy);
		...
		private void DeltaEnemies( Type oldType, Type newType )
		{
			foreach ( Mobile m in this.GetMobilesInRange( 18 ) )
			{
				...
				if ( t == oldType || t == newType ) {
					...
					if ( ns != null ) {
						if ( ns.StygianAbyss ) {
							ns.Send( new MobileMoving( m, Notoriety.Compute( this, m ), this ) );
                            // Scriptiz 3e paramètre pour le système d'hallucinations
						} else {
							ns.Send( new MobileMovingOld( m, Notoriety.Compute( this, m ), this ) );
                            // Scriptiz 3e paramètre pour le système d'hallucinations
						}
					}
				}
		...
		public override TimeSpan GetLogoutDelay()
		{
            ...
            // Scriptiz : ajout pour les GMs qui ont des persos joueurs
            if (Account != null && Account.AccessLevel > AccessLevel.Player)
                return TimeSpan.Zero;
        ...
        public double CalculateHealAddiction(BasePotion potion)
        {
            if (Addiction == null)
                Addiction = new double[Enum.GetValues(typeof(PotionEffect)).Length];

            double CurrentAddiction = Addiction[(int)potion.PotionEffect];
            double HealConsumption = CurrentAddiction + Addiction[14]*0.08 + Addiction[15]*0.1 + Addiction[16]*0.14;

            return HealConsumption;
        }

        public double[] CalculateAgilityAddiction(BasePotion potion)
        {
            if (Addiction == null)
                Addiction = new double[Enum.GetValues(typeof(PotionEffect)).Length];

            double CurrentAddiction = (Addiction[(int)potion.PotionEffect]);
            double GlobalAddiction = CurrentAddiction + Addiction[4] * 0.1 + Addiction[5] * 0.14;
