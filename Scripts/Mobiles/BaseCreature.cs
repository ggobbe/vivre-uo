...
namespace Server.Mobiles
{
    ...
    public partial class BaseCreature : Mobile, IHonorTarget, IQuestGiver
    {
        ...
        private Int32 m_specificHue;
        private bool m_hasSpecificHue = false;
        ...
        public virtual Int32 SpecificHue
        {
            get
            {
                return m_specificHue;
            }
            set
            {
                m_hasSpecificHue = true;
                m_specificHue = value;
            }
        }
        public virtual bool HasManaOveride { get { return false; } }
        public virtual bool HasSpecificHue { get { return m_hasSpecificHue; } }
                    ...
                    if (Core.AOS && (holding is SkinningKnife /* TODO: || holding is ButcherWarCleaver || with is ButcherWarCleaver */ ))
                    {
                        ...
                        if (HasSpecificHue)
                        {
                            switch (HideType)
                            {
                                ...
                                case HideType.Spined: leather = new SpinedLeather(hides, SpecificHue); break;
                                case HideType.Horned: leather = new HornedLeather(hides, SpecificHue); break;
                                case HideType.Barbed: leather = new BarbedLeather(hides, SpecificHue); break;
                                case HideType.Daemon: leather = new DaemonLeather(hides); break;
                    ...
                    else
                    {
                        if (HasSpecificHue)
                        {
                            if (HideType == HideType.Regular)
                                ...
                                corpse.DropItem(new SpinedHides(hides, SpecificHue));
                            else if (HideType == HideType.Horned)
                                corpse.DropItem(new HornedHides(hides, SpecificHue));
                            else if (HideType == HideType.Barbed)
                                corpse.DropItem(new BarbedHides(hides, SpecificHue));
		...
		*/
        ...
        public bool PackArmor(int minLevel, int maxLevel, double chance)
        {
            ...
            if (Core.AOS)
            {
                ...
                bool isQuality = Utility.RandomBool();
                ...
                if (item is BaseWeapon)
                {
                    if (isQuality == true)
                        ...
                        ((BaseWeapon)item).Quality = WeaponQuality.Low;
                }
        ...
        public virtual void OnKilledBy(Mobile mob)
        {
            ...
            XmlQuest.RegisterKill(this, mob);// Vinds : kill register pour xmlspawner
            ...
            // Scriptiz : Malas counters
            if (Map != Map.Malas) return;
            PlayerMobile pm = mob as PlayerMobile;
            if (pm == null || pm.AccessLevel > AccessLevel.Player) return;
            Alignments.Instance.incKills(pm.Alignment, this.Player);
        }
        ...
        public int CalculatePenalty(Mobile healer)
        {
            int PenaltySkill = (int)Math.Ceiling(RawStr / 100.0) * (int)Math.Ceiling(Deaths / 2.0);

            PenaltySkill -= (int)(healer.Skills[SkillName.Veterinary].Value - 90);

            foreach (Item i in GetItemsInRange(5))
            {
                if (!(i is Corpse)) continue;	// si l'objet n'est pas un corps on ne traite pas cet objet mais on continue
                Corpse c = i as Corpse;	// on caste en corp
                if (c.Owner == this)	// si le owner du corp est le dragon on arrête la boucle et on met le corps trouvé dans corp
                {
                    PenaltySkill--;
                    break;
                }
    ...
    public class LoyaltyTimer : Timer
    {
        ...
        protected override void OnTick()
        {
                ...
                foreach (Mobile m in World.Mobiles.Values)
                {
                    ...
                    if (m is BaseCreature)
                    {
                        ...
                        else if (c.Controlled && c.Commandable)
                        {
                            ...
                            if (c.Map != Map.Internal && c.ControlMaster != null)
                            {
                                c.Loyalty -= (BaseCreature.MaxLoyalty / 10);

                                // Scriptiz : les dragons perdent plus !
                                Type type = this.GetType();
                                if (type == typeof(Dragon) || type == typeof(Drake) || type == typeof(WhiteWyrm) || type == typeof(AncientWyrm) || type == typeof(ShadowWyrm) || type == typeof(GreaterDragon))
                                {
                                    c.Loyalty -= (BaseCreature.MaxLoyalty / 5);
                                }
            ...
            {
                Logging.DebugLog(String.Format("[BaseCreature] d = {0}\n\t{1}\n", d, e.Message));
            }
        }
    }
}