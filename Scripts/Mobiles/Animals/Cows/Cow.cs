	...
	public class Cow : BaseCreature
	{
		...
		public bool TryMilk( Mobile from )
		{
			if ( !from.InLOS( this ) || !from.InRange( Location, 2 ) )
				from.SendMessage( "Vous ne pouvez traire la vache d'ici" ); // You can not milk the cow from this location.
			else if ( Controlled && ControlMaster != from )
				from.SendMessage( "La vache échappe agilement à votre tentative de la traire" ); // The cow nimbly escapes your attempts to milk it.
			else if ( m_Milk == 0 && m_MilkedOn + TimeSpan.FromDays( 1 ) > DateTime.Now )
				from.SendMessage( "Cette vache n'a plus de lait, réessayez demain" ); // This cow can not be milked now. Please wait for some time.
			else
			{
				if ( m_Milk == 0 )
					m_Milk = 4;
                ...
                // Scriptiz : s'il y a un taureau dans les environs, il est possible qu'il charge
                if (Utility.Random(1, 10) == 1)   // 1 chance sur 10
                {
                    foreach (Mobile m in this.GetMobilesInRange(10))
                    {
                        if (m != null && m is Bull)
                        {
                            m.Emote("*Mugit et charge " + from.Name + "*");
                            m.Attack(from);
                        }
                    }
                }
