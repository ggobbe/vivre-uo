...
namespace Server.Multis
{
	...
	public abstract class BaseBoat : BaseMulti
	{
		...
		public override bool Contains( int x, int y )
		{
            ...
            // Scriptiz s'il s'agit d'un bateau orc, on considère les statics comme dedans
            if (this is OrcBoat)
            {
                OrcBoat ob = (OrcBoat)this;
                foreach (BoatStatic bs in ob.StaticParts)
                {
                    if (bs == null) continue;
                    if (bs.X == x && bs.Y == y) return true;
                }
            }
			...
			foreach ( object o in map.GetObjectsInBounds( new Rectangle2D( X + mcl.Min.X, Y + mcl.Min.Y, mcl.Width, mcl.Height ) ) )
			{
				...
				else if ( o is Mobile )
				{
					...
					if ( Contains( m ) )
						list.Add( m );
                        /*
                    // Scriptiz : on bouge aussi les joueurs sur des boatstatic
                    else if (this is OrcBoat)
                    {
                        OrcBoat ob = (OrcBoat)this;
                        ...
                        foreach(BoatStatic bs in ob.StaticParts)
                        {
                            if (bs.X == m.X + xOffset && bs.Y == m.Y + yOffset)
                            {
                                m.Location = new Point3D(m.X + xOffset, m.Y + yOffset, m.Z + zOffset);
                                break;
                            }
