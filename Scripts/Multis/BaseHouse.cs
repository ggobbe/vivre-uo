...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		public const bool DecayEnabled = false; // Scriptiz : on réactivera pour si + de 5 maisons !
		...
		public virtual TimeSpan DecayPeriod{ get{ return TimeSpan.FromDays( 31.0 ); } } // Scriptiz : default = 5 days
        ...
        public virtual DecayType DecayType
        {
            get
            {
                // Scriptiz : les maisons sur des copies de Trammel et Felucca ne decay pas !
                if (this.Map.MapID >= 6)
                ...
                // Scriptiz : c'est désormais la maison la plus vieille qui est auto-refresh
                BaseHouse oldest = null;

                for (int i = 0; i < allHouses.Count; ++i)
                {
                    BaseHouse check = allHouses[i];

                    if (oldest == null || IsOlder(check, oldest))
                        oldest = check;
                }

                if (this == oldest)
                    return DecayType.AutoRefresh;
                // Scriptiz : fin changement oldest au lieu de newest
        ...
        // Scriptiz : oldest house (build time) DecayType is AutoRefresh instead of the newest house !
        public bool IsOlder(BaseHouse check, BaseHouse house)
        {
            DateTime checkTime = (check.LastTraded > check.BuiltOn ? check.LastTraded : check.BuiltOn);
            DateTime houseTime = (house.LastTraded > house.BuiltOn ? house.LastTraded : house.BuiltOn);
		...
		public bool CheckAccessibility( Item item, Mobile from )
		{
            ...
            if (!IsLockedDown(item))
                ...
                return true;
            // Scriptiz : pour pouvoir utiliser les poteaux d'arnachement dans les maisons !
            else if (item is PoteauArnachement)
                return IsCoOwner(from);
            //Plume : Pour la Slayer Forge
            else if (item is SlayerForge)
                return true; 
		...
		public static bool HasHouse( Mobile m )
		{
            ...
            // Scriptiz : nombre de maisons illimité !
            return false;
		}
