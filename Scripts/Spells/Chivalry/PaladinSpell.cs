...
namespace Server.Spells.Chivalry
{
	public abstract class PaladinSpell : Spell
	{
        ...
        public abstract string ArtedarMantra { get; }
		...
		public override void SayMantra()
		{
            ...
            if ( Caster is PlayerMobile && ((PlayerMobile)Caster).IsArtedarKnight)  //Vinds : power words différents pour artedar                       
                 Caster.Say(ArtedarMantra);
		...
		public virtual void SendCastEffect()
		{
           ...
           Caster.FixedEffect(0x37C4, 10, (int)(GetCastDelay().TotalSeconds * 28), (Caster is PlayerMobile && ((PlayerMobile)Caster).IsArtedarKnight) ? 27 : 4, 3); // vinds : switch cosmétique suivant le dieu
		...
		public static int ComputePowerValue( Mobile from, int div )
		{
            ...
            if (from is PlayerMobile && ((PlayerMobile)from).IsArtedarKnight) // vinds : la fame est plus importante pour artedar que le karma bon ou mauvais
            {
                int v = (int)Math.Sqrt(1.75 * (from.Fame) + 20000 + (from.Skills.Chivalry.Fixed * 10));
                if (v > 204)  // même power cap que pour la chivalry classique
                    v=204;
                return v / div;
