...
namespace Server.Spells.Seventh
{
	public class PolymorphSpell : MagerySpell
	{
        ...
        // Scriptiz : le sort ne coute pas de réactifs si utilisé par une fée pour se transformer en fée
        public override bool ConsumeReagents()
        {
            // Scriptiz : le sort ne bouffe pas de réactifs si c'est une fée qui se transforme en fée
            if (m_NewBody == 128 && Caster is PlayerMobile && ((PlayerMobile)Caster).IsFairy)
                return true;

            return base.ConsumeReagents();
        }
			...
			else if ( CheckSequence() )
			{
				if ( Caster.BeginAction( typeof( PolymorphSpell ) ) )
				{
					if ( m_NewBody != 0 )
					{
                        ...
                        // Scriptiz : Blinght est un poulet vert !
                        if (m_NewBody == 208 && Caster.Name == "Blinght" && Caster.Account != null && Caster.Account.Username == "Blinght")
                            Caster.HueMod = 70;
