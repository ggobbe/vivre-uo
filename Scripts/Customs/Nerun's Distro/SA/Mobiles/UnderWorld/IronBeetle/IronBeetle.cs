...
namespace Server.Mobiles
{
    ...
    public class IronBeetle : BaseCreature
    {
        ...
        {
            Str = (int)(Str*SummonScalar);
            HitsMaxSeed = (int)(HitsMaxSeed * SummonScalar);
            for (int i = 0; i < this.Skills.Length; i++)
            {
                Skill skill = (Skill)this.Skills[i];

                if (skill.Base > 0.0)
                    skill.Base *= SummonScalar;
            }
        }
