/* 
 * Updated to RunUO 2.1 by Scriptiz
 ...
 *
 * History since updated to RunUO 2.1 :
 * 	2.1b : Enabling player's context menus is now working well
 *	2.1 : Complete script ported to RunUO 2.1
 * 
 *********************************
 ...
 * 
 * To add context menu just do this : 
 * 1) add "using Server.Commands;" to your playermobile file top (near others using)
 * 2) in your playermobile add "JailSystem.GetContextMenus(from, this, list);" right after 
 ...
 * should look something like this
    ...
    public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
	{
        ...
        // Scriptiz : Here is what you have to add !
...
namespace Server.Commands
{
    public class JailSystem
    {
        ...
        const string scriptVersion = "2.1b"; // Scriptiz : updated to RunUO 2.1 thus increasing script version
        ...
        {
            statusCommand = "temps prison";
            timeCommand = "quelle heure";
            ...
            defaultRelease.Add(new Point3D(3503, 2574, 14));
            useLanguageFilter = true;
            foulJailorName = "Language Auto Jailor";
            allowStaffBadWords = true;
            badWords.Clear();
            badWords.Add("fuck");
            badWords.Add("go jail");
            FoulMouthJailTimes.Clear();
            FoulMouthJailTimes.Add(new TimeSpan(0, 0, 5, 0));
            ...
            AllowStaffOOC = true;
            oocJailTimes.Clear();
            oocJailTimes.Add(new TimeSpan(0, 0, 5, 0));
        ...
        {
            // Scriptiz : on charge les paramètres par défaut d'office !
            JailSystem.defaultSettings();
            return;
        ...
        public string Name
        {
            get
            {
                // Scriptiz : bizarre que le nom soit null
                if (m_name == null)
                {
                    Logging.DebugLog(String.Format("JailSystem - m_name est null"));
                    return "<Null>";
                }
        ...
        public static void GetContextMenus(Mobile from, Mobile player, List<ContextMenuEntry> list)
        {
            ...
            if (from.AccessLevel >= AccessLevel.Counselor)
                list.Add(new MacroEntry(from, player));
        }
    ...
    public class JailReviewGump : Gump
    {
        ...
        public JailReviewGump(Mobile from, Mobile m, int id, ArrayList warnings)
            : base(1, 30)
        {
            buildit(from, m, id, warnings, "");
        }

        public JailReviewGump(Mobile from, Mobile m, int id, ArrayList warnings, string note)
            : base(1, 30)
        {
            buildit(from, m, id, warnings, note);
        }

        public JailReviewGump(Mobile from, Mobile m, int id, ArrayList warnings, bool showRelease)
            : base(1, 30)
        {
            buildit(from, m, id, warnings, "", true, showRelease);
        }

        public JailReviewGump(Mobile from, Mobile m, int id, ArrayList warnings, string note, bool showRelease)
            : base(1, 30)
        {
    ...
    public class JailWarnGump : Gump
    {
        ...
        private ArrayList m_warn;
        private int m_id;
        public JailWarnGump(Mobile from, Mobile m, string why, int id, ArrayList warnings)
            : base(100, 40)
        {
    ...
    {
        public JailWarningGump(Mobile from, Mobile m, string why)
            : base(70, 40)
        {
    ...
    public class MacroEntry : ContextMenuEntry
    {
        private Mobile m_gm;
        private Mobile m_player;
        private JailSystem js;
...
namespace Server.Items
{
    ...
    public class SmokingFootGear : Item
    {
        ...
        private class sTimer : Timer
        {
            ...
            int tI = 1;
            public sTimer(SmokingFootGear i)
