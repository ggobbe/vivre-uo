	   ...
	   if(index < MaxTraces){
        ...
        private class GlobalSectorTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                if (NetState.Instances == null) return;
                ...
                foreach (NetState state in NetState.Instances)
                {
                    ...
                    if (m != null && (m.AccessLevel <= SmartSpawnAccessLevel || !m.Hidden))
                    {
                        ...
                        if (m.Map != null && m.Map != Map.Internal)
                        {
                            ...
                            // Scriptiz : voyons voir ce qui cloche
                            if (m.Map.MapID >= GlobalSectorTable.Length)
                            {
                                using (StreamWriter sw = new StreamWriter("debug_scriptiz.txt", true))
                                {
                                    string error = String.Format("{0} : MapID ({1}:{2}) > GlobalSectorTable.Length ({3})", DateTime.Now, m.Map.Name, m.Map.MapID, GlobalSectorTable.Length);
                                    sw.WriteLine(error);
                            ...
                            {
                                // Scriptiz : voyons voir ce qui cloche
                                if (!GlobalSectorTable[m.Map.MapID].Contains(s))
                                {
                                    using (StreamWriter sw = new StreamWriter("debug_scriptiz.txt", true))
                                    {
                                        string error = String.Format("{0} : GlobalSectorTable doesn't contains sector {1}:{2} ({3},{4})", DateTime.Now, m.Map.Name, m.Map.MapID, s.X, s.Y);
                                        sw.WriteLine(error);
                                        Console.WriteLine(error);
                                    }
                                    return;
